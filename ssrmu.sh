...
ss_link_qr(){
	...
	SSurl="ss://${SSbase64}"
	SSQRcode="http://dev.yunnan.ws/qr/qr.php?text=${SSurl}"
...
ssr_link_qr(){
	...
	SSRQRcode="http://dev.yunnan.ws/qr/qr.php?text=${SSRurl}"
...
Set_config_user(){
	...
	stty erase '^H' && read -p "(默认: tekin):" ssr_user
	[[ -z "${ssr_user}" ]] && ssr_user="tekin"
