...
Install_MariaDB103() {
  ...
  if [ "${dbInstallMethods}" == "1" ]; then
    tar zxf mariadb-${mariadb103_version}-${GLIBC_FLAG}-${SYS_BIT_b}.tar.gz
    mv mariadb-${mariadb103_version}-*-${SYS_BIT_b}/* ${mariadb_install_dir}
    ...
    tar xvf mariadb-${mariadb103_version}.tar.gz
    pushd mariadb-${mariadb103_version}
  ...
  if [ -d "${mariadb_install_dir}/support-files" ]; then
    ...
    if [ "${dbInstallMethods}" == "1" ]; then
      rm -rf mariadb-${mariadb103_version}-*-${SYS_BIT_b}
    elif [ "${dbInstallMethods}" == "2" ]; then
      rm -rf mariadb-${mariadb103_version}
    fi
  else
