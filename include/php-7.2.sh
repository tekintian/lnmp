...
EOF
  ...
  if [[ ! $Apache_version =~ ^[1-2]$ ]] && [ ! -e "$apache_install_dir/bin/apxs" ]; then
    ...
    sed -i "s@^# Provides:.*@# Provides:          php72-fpm@" /etc/init.d/php72-fpm
    chmod +x /etc/init.d/php72-fpm
    [ "$OS" == 'CentOS' ] && { chkconfig --add php72-fpm; chkconfig php72-fpm on; }
    [[ $OS =~ ^Ubuntu$|^Debian$ ]] && update-rc.d php72-fpm defaults
